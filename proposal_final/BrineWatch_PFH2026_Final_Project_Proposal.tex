...
Project collaborators & Lorenzo Bacchiani \textperiodcentered\ Andrea Melis \textperiodcentered\ University of Bologna \\[1.5pt]
...
\begin{tikzpicture}[x=1cm,y=1cm,font=\scriptsize\sffamily]
  ...
  \node[anchor=south west,align=left,color=navy] at (11.1,3.42)
      {\textbf{BlueROV2} + owned imaging sonar\\[-1pt]{\color{muted}+ planned conductivity--temperature payload}};
...
\begin{tikzpicture}[font=\scriptsize\sffamily,
  ...
  \node[hw,minimum width=4.35cm] (sonar) at (0,-1.28)
    {\textbf{\color{deepblue}\faWifi~~On-vehicle sonar (owned)}\\imaging sonar};
...
\end{tikzpicture}
\caption{\textbf{System overview.} Solid blue vehicle panels: hardware owned today. Dashed panel:
the planned calibrated CT sensor. Teal panels: mission software already
...
\small
\begin{itemize}
  \item BlueROV2 and on-vehicle sonar hardware owned by the team
...
BrineWatch separates the one-time cost of completing the sensing and navigation system from the much lower cost of repeating each mission. The team already owns the BlueROV2, including its tether, camera, lighting and basic onboard instrumentation, together with the on-vehicle sonar hardware, including a side-scan unit, so the project budget does not include the purchase of a new underwater vehicle or sonar. The estimated USD 5k--9k core completion cost covers the calibrated CT payload, mounting, interfaces, integration, calibration and essential spares, while a more precise underwater navigation solution, such as DVL/INS and, where absolute georeferencing is required, USBL, will be specified separately according to the accuracy required for field 2-D and 3-D mapping. Once the system is completed, recurring costs are limited mainly to local transport, consumables, equipment wear and the operational team; deployment can take place from shore or, when necessary, from a small locally chartered boat, without purchasing or permanently operating a fully equipped survey vessel.
...
\small Calibrated CT payload, mounting and interfaces, integration, calibration
...
[6] Cerulean Sonar: Omniscan 450 imaging-sonar product family.\par
