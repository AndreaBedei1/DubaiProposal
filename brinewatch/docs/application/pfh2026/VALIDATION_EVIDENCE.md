...
| 4 | Runtime-spawned props are invisible to the OFFICIAL sonar but VISIBLE to the CUSTOM fork | official: `assets/sonar/gate/` (bit-identical present/absent frames, z=0.00). custom: `outputs/sonar_truth/custom_run1/` (BOX/CYL/OUTFALL all visible, 30–34k changed bins) — the fork rebuilds the octree at runtime (SONAR_VALIDATION §0) | official-engine limitation lifted by the custom fork |
